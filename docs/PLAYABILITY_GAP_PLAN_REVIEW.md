...
- **Phase 17 vs Phase 20:** _Resolved._ Phase 17 is now split into 17a (hunting/resonance — independent) and 17b (discipline activation — blocked on Phase 20's `PoolDefinitionJson` migration + importer). The dependency graph and gap table both reflect this. Phase 20's data-model slice (migration + JSON column) must merge before 17b can start; Phase 20's content pass (populating `PoolDefinitionJson` values) can follow in parallel with 17b work.

- **Phase 18 vs Phase 20:** _Resolved._ A shared `DegenerationCheckRequired(CharacterId, DegenerationReason)` domain event is defined once in the Domain layer (see the shared-event block added before Phase 18 in the plan). Phase 20 raises it with `Reason = CrúacPurchase` before Phase 18's roll UI exists — at that point the handler creates a Glimpse notification only and does **not** block the purchase. The full degeneration roll UI is wired when Phase 18 ships. No duplicate event types; no circular dependency.

- **Vitae-zero frenzy (Phase 16):** _Resolved._ `VitaeDepletedEvent` is in-process only (no cross-service guarantee on restart — acceptable for the same app process). The plan requires tilt application inside a single `SaveChangesAsync` transaction with a guard plus **EF optimistic concurrency** (row version or a unique index on `CharacterId + TiltType + IsActive`) so concurrent `VitaeDepletedEvent` handlers cannot insert duplicate active tilts.

**Phase 15 — combat**

- **Scope of first slice:** _Resolved._ MVP boundary is explicit in the plan: melee + generic weapon profile only. Ranged/firearms, improvised weapons, and touch attacks are documented as follow-on slices in `rules-interpretations.md` when Phase 15 ships.

- **Defense:** _Resolved._ `AttackService` reads `character.Defense` from the existing derived stat (already calculated including armor modifiers from Phase 11). Defense vs. firearms and unaware-target rules are recorded in `rules-interpretations.md`. Dodge as a declared action is out of scope for Phase 15.

- **Aggravated sources:** _Resolved._ `DamageSource` enum is added to `AttackResult` from day one: Bashing, Lethal, Aggravated, Fire, Sunlight, Weapon. Phase 16's `FrenzyService` reuses `Fire` and `Sunlight` tags for Rötschreck triggers. No retrofitting needed.

**Phase 16 — frenzy / torpor**
...
- **Social maneuvering:** _Acknowledged._ The plan has the interceptor contest the initiator using Manipulation + Persuasion; tie-breaking and other edge cases are left to the Phase 19 rules-log task (`rules-interpretations.md` at ship), not spelled out in the architectural section.

**Phase 20 — disciplines**

- **Soft gates and audit:** _Resolved._ For every purchase where `AcquisitionAcknowledgedByST = true`, append the canonical suffix to `XpLedgerEntry.Notes`: space + pipe + space + `gate-override stUserId={userId} {timestamp:O}` (see Phase 20 architectural decisions for a full example). Same string appears in the Phase 20 task list; record the format in `rules-interpretations.md` when implementing.
...
1. **Fix dependency narrative:** _Done._ Phase 17 split into 17a/17b; dependency graph updated with three parallel tracks; Phase 17b explicitly depends on Phase 20 schema.

2. **Align with existing modifier infrastructure:** _Done._ Phase 15 now specifies `WoundPenaltyResolver` returns a `PassiveModifier(Target = ModifierTarget.WoundPenalty)` injected into `ModifierService.GetModifiersForCharacterAsync` — the same aggregation loop as Coils and equipment. No separate resolver call; no `TraitResolver` change.

3. **Verify seed path:** _Acknowledged in plan._ Phase 20 task list includes: importer idempotency (upsert by name), integration test coverage before `DisciplineSeedData.cs` is deleted, and rollback story (if `Disciplines.json` fails to parse, `DbInitializer` throws and startup fails fast — same behavior as all other JSON importers).

4. **Single event contract for degeneration:** _Done._ `DegenerationCheckRequired(CharacterId, DegenerationReason)` is defined in a dedicated shared-event block between Phases 17b and 18. Both Phase 18 and Phase 20 reference it. `DegenerationReason` covers `StainsThreshold` and `CrúacPurchase`; future reasons (e.g., diablerie) extend the enum.

5. **Combat MVP boundary:** _Done._ Explicit "Phase 15 MVP boundary" note added to the Phase 15 architectural decisions: first shippable slice = melee attack + health overflow + wound penalty + armor mitigation. Specialty damage tags (`DamageSource`) added from day one to avoid retrofitting.

6. **Accessibility / Glimpse patterns:** _Done._ Added to non-goals: all new UI in Phases 15–18 must reuse existing Glimpse patterns, `DiceRollerModal`, and Phase 13 ARIA announcer. No new front-end dependencies.
...
1. **Background scheduling** — Phase 16 uses `TorporIntervalService : BackgroundService` (not `IBackgroundJobService`). A redundant Phase 16 task line that still referenced `IBackgroundJobService` was removed from the plan so tasks match the architecture.

2. **Phase 17 vs 17b wording** — Gap row 25, Phase 20 objective, Current State table, and `DisciplineActivationService` bullet use **17b** where appropriate.

3. **Ledger audit format** — Single canonical suffix: ` | gate-override stUserId={userId} {timestamp:O}` (space-pipe-space before `gate-override`), with example in Phase 20 architectural decisions.
