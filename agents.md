...
3. For **Phase 20 performance / refactor / UI-consistency** work, read [`docs/plan-improvement.md`](./docs/plan-improvement.md#start-here-agents) from **Start here (agents)** onward. **Waves 1–4 are delivered** — treat that doc as a **delivery record** and optional P3 backlog unless you are explicitly implementing an **open** row in its Status summary. Do not confuse **Wave** numbers with `mission.md` phase numbers.
...
**Phases 14–16b, 17, 18, 19, and 19.5 are complete** (combat & wounds, frenzy & torpor, feeding / hunting, discipline activation, humanity & conditions, **Wider Web** edge systems & seed catalogs, discipline acquisition, **blood sorcery rules completion** — Crúac / Theban / Necromancy: extended rites, ritual Conditions, informational Potency, canonical ritual JSON in `SeedSource`, sacrament UX, backlog verification). Record: [`docs/mission.md`](./docs/mission.md) Phase 19.5 + [`docs/rules-interpretations.md`](./docs/rules-interpretations.md) Phase 19.5. **Phase 20 — The Global Embrace** — **roadmap checklist complete** (2026-04-04): Discord session incoming webhooks + SignalR receive-size tuning; discretionary P3+ items remain in [`docs/plan-improvement.md`](./docs/plan-improvement.md#start-here-agents); **not** near-term: i18n, public REST API — [`docs/mission.md`](./docs/mission.md). **Phase 20 technical polish — Waves 1–4 are delivered** (2026-04-03): seed pipeline, reference cache, `CharacterQueryService`, modifier providers, rite strategies, `CharacterDetails` partials, loading/error UX, `CharacterAdvancement` sections, `DanseMacabre` tabs, `EncounterManager` **`EncounterParts/`** + NpcPicker/SmartLaunch partials (**#20** done) — see [`docs/mission.md`](./docs/mission.md) Phase 20 and [`docs/plan-improvement.md`](./docs/plan-improvement.md#start-here-agents). **Optional P3** backlog is mainly **`InitiativeTracker`** markup splits (**#19**); start at **Start here (agents)** in that doc. Phase 18 delivery record: [`docs/mission.md`](./docs/mission.md) (section **Phase 18: The Wider Web**). Phase 16b reference: [`docs/phase16b-the-discipline-engine.md`](./docs/phase16b-the-discipline-engine.md). Phases 13, 12, and 8–11 are complete.
